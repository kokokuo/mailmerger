...
import io
import os
from app.mailagent import MailAgent
import smtplib
import pandas as pd

# 信件排版可以用 http://htmlformatter.com/
# 若使用 https://codebeautify.org/htmlviewer/ 會跑版
def parse_excel(filepath):
    # sheet_name = raw_input('請輸入要讀取的 Sheet >> ').strip().decode('utf8')
    
    sheet_name = u'sheet1'
    print u'您要讀取的為 {} 試算表'.format(sheet_name)
    ...
    # 顯示指定的 column 的所有資料
    # 要找出的 key 值
    df_name_email_rows = sheet_df[[u'hotel_name', u'email']]
    num = 1
