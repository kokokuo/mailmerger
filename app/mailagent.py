...
class MailAgent(object):
    ...
    def __mapping_variable(self, template_vars, mapping_data):
        ...
        if not_include_template_vars:
            print u'此筆資料中的欄位: {}, 不存在於信件樣板中，因此無法被匹配填入'.format(', '.join(not_include_template_vars))
        if not_mapped_var_in_template:
            print u'信件中的替換關鍵字 {} 沒有資料對應到'.format(', '.join(not_mapped_var_in_template))
    ...
    def send_mapped_data_email(self, mapping_data):
        ...
        if self.__mapping_variable(template_vars, mapping_data):
            ...
            if u'email' in mapping_data.keys() and (type(mapping_data[u'email']) is unicode):
                receivers = [mapping_data[u'email']]
                # receivers = ['v6610688@gmail.com']
                msg_root['To'] = ','.join(receivers)
            else:
                print '民宿：', mapping_data[u'hotel_name'],  "「不存在信箱...繼續下一封」"
                print '==================================s================================'
                return
            ...
            try:
                ...
                print '民宿：', mapping_data[u'hotel_name'], '「Done, 郵件發送成功！」'
                print '=================================================================='
            except smtplib.SMTPException:
                print "Error", '民宿：', mapping_data[u'hotel_name'], "「無法發送郵件.......」" 
                return
